...
fragment float4 FragmentStage(v2f in [[stage_in]],
                              ...
                              texture2d_array<float> lod3Tex [[texture(3)]],
                              texture2d_array<float> lod4Tex [[texture(4)]],
                              constant FrameConstants* frameConstants[[buffer(0)]])
{
    const float2 gridDims = float2(1.0, 3.0); 
    
    float colFloat = in.uv.x * 5.0;
    ...
    else if (colIndex == 3) columnColor = lod3Tex.sample(s, tileUV, sliceIndex);
    else                    columnColor = lod4Tex.sample(s, tileUV, sliceIndex);
